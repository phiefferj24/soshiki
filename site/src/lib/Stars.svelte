<script lang="ts">
    ...
    function mouseLeft() {
        starValues = [
            Math.min(score, 2),
            Math.max(Math.min(score, 4) - 2, 0),
            Math.max(Math.min(score, 6) - 4, 0),
            Math.max(Math.min(score, 8) - 6, 0),
            Math.max(Math.min(score, 10) - 8, 0),
        ]
    }
...
<div class="stars" bind:this={starsDiv} on:click={event => mouseClicked(event)} on:mouseleave={mouseLeft}>
    {#each starValues as value, index}
        <i class="f7-icons star" bind:this={stars[index]}>{value === 0 ? "star" : (value === 1 ? "star_lefthalf_fill" : "star_fill")}</i>
    {/each}
</div>
