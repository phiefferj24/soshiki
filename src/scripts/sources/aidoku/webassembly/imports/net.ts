...
export class Net implements Imports {
    ...
    defaultUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.150 Safari/537.36 Edg/88.0.705.63";
    ...
    isRateLimited(): boolean {
        return this.rateLimit > 0 && new Date().getTime() - (this.lastRequestTime?.getTime() ?? 0) < this.period && this.passedRequests >= this.rateLimit;
    }

    incrementRequest(): void {
        if (new Date().getTime() - (this.lastRequestTime?.getTime() ?? new Date().getTime() - 60 * 1000) < this.period) {
            this.passedRequests++;
        } else {
    ...
    set_header(descriptor: number, key: number, keyLen: number, value: number, valueLen: number): void {
        if(descriptor >= 0 && keyLen > 0 && valueLen > 0 && this.wasm.requests.has(descriptor)) {
            this.wasm.requests.get(descriptor)!.headers.set(this.wasm.readString(key, keyLen), this.wasm.readString(value, valueLen));
        }
    }
    ...
    send(descriptor: number): void {
        if(this.wasm.requests.has(descriptor) && this.wasm.requests.get(descriptor)!.url) {
            ...
            for (let [key, value] of xhr.getAllResponseHeaders().split("\n").map(line => line.split(": ")).filter(line => line.length == 2)) {
                headers.set(key, value);
            }
    ...
    get_data(descriptor: number, buffer: number, size: number): void {
        ...
        if (this.wasm.requests.has(descriptor) && this.wasm.requests.get(descriptor)!.response?.data) {
            ...
            if (response.bytesRead + size > response.data!.length) { return }
...
export class ResponseObject implements KVCObject {
    ...
    constructor(status: Optional<number> = null, data: Optional<Uint8Array> = null, headers: Map<string, string> = new Map()) {
        this.status = status;
        this.data = data;
    ...
    valueByPropertyName(propertyName: string) {
        switch (propertyName) {
            ...
            case "status_code": return this.status;
...
export class RequestObject implements KVCObject {
    ...
    constructor(id: number, url: Optional<string> = null, method: Optional<HttpMethod> = null, headers: Map<string, string> = new Map(), body: Optional<Uint8Array> = null) {
        this.id = id;
        this.url = url;
        this.method = method;
        this.headers = headers;
