...
function proxyRequest(req, res, proxy) {
  ...
  if (typeof params !== 'undefined') {
    ...
    for (let p of split) {
      ...
      if (k === "soshiki_set_header") {
        let v1 = decodeURIComponent(v);
        let v2 = v1.substring(0, v1.indexOf(":"));
        let v3 = v1.substring(v1.indexOf(":") + 1);
        headers[v2] = v3;
      }
    }
    req.url = req.url.replace(/soshiki_set_header=[^&]*(&?)/g, "");
    if (req.url.split("?")[1].length === 0) req.url = req.url.split("?")[0];
  }
